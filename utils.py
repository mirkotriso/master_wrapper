...
def check_strings(data):
    if any(isinstance(x, list) for x in data):
        ...
        if any(isinstance(x, (float, int)) for x in merged):
            merged = [str(i) for i in merged]
            data = [merged[i:i+len(merged)/len(data)] for i in range(0, len(merged), len(merged)/len(data))]
            return data
        else:
            return data
    if any(isinstance(x, (float, int)) for x in data):
